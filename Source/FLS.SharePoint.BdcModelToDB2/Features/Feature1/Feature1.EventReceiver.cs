...
namespace FLS.SharePoint.BdcModelToDB2.Features.Feature1
{
    public class ImportModelReceiver : SPFeatureReceiver
    {
        ...
        private void ImportTheSavedModel(string modelContents, Dictionary<string, LobSystemAssemblies> lobSystemsAssemblies, bool incrementalUpdate)
        {
            ...
            var collection = model.AllReferencedLobSystems;
            LobSystemAssemblies assemblies;
            foreach (var lobSystem in collection)
            {
                if (!lobSystemsAssemblies.TryGetValue(lobSystem.Name, out assemblies)) continue;
        ...
        {
            var modelList = this.modelDocument.GetElementsByTagName("Model");
            if (modelList == null || modelList.Count != 1)
            {
                throw new InvalidDataException(string.Format(CultureInfo.CurrentCulture,
                                                             "Model file {0} is not valid. 'Model' root element not found.",
                                                             this.modelFileName));
            }

            var nameAttribute = modelList[0].Attributes["Name"];
            if (nameAttribute == null)
            {
                throw new InvalidDataException(string.Format(CultureInfo.CurrentCulture,
                                                             "Model file {0} is not valid. 'Model' root element does not have a 'Name' attribute.",
                                                             this.modelFileName));
            }
        ...
        private string GetModelFilePath(SPFeatureProperty featureProperty, string featureFolder)
        {
            ...
            catch (ArgumentException ex)
            {
                throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
                                                          "Feature property 'ModelFileName' has invalid value. Feature activation failed with the following message: {0}",
                                                          ex.Message));
            }
        ...
        private static string GetFeatureFolder(SPFeatureDefinition featureDefinition)
        {
            ...
            var featureFolder = Path.Combine(SPUtility.GetGenericSetupPath(@"Template\Features"), featureDefinition.DisplayName);
        ...
        {
            var fileNameWithoutExtension = Path.GetFileNameWithoutExtension(this.modelFileName);
            var exportedModelFilePath = string.Format(CultureInfo.InvariantCulture,
                "{0}{1}.xml",
                Path.Combine(targetDirectory, fileNameWithoutExtension),
                DateTime.Now.ToString(FileSuffix, CultureInfo.InvariantCulture));
            using (var sw = new StreamWriter(exportedModelFilePath))
            {
            ...
            string featureFolder)
        {
            ...
            var lobSystemList = this.modelDocument.GetElementsByTagName("LobSystem");

            var lobSystemProperties = new Dictionary<string, string>(lobSystemList.Count);

            foreach (XmlNode node in lobSystemList)
            {
                var lobSystemNameAttribute = node.Attributes["Name"];
                //If lobSystem does not have a Name, import will fail with XSD validation.
                if (lobSystemNameAttribute != null)
                {
                    var lobSystemName = lobSystemNameAttribute.Value;
                    if (featurePropertyCollection[lobSystemName] != null &&
                        !String.IsNullOrEmpty(featurePropertyCollection[lobSystemName].Value))
                    {
                        var lobSystemProperty = featurePropertyCollection[lobSystemName].Value;
                        lobSystemProperties.Add(lobSystemName, lobSystemProperty);
                    }
                }
            }

            var lobSystemAssemblies = new Dictionary<string, LobSystemAssemblies>();

            foreach (var lobSystemName in lobSystemProperties.Keys)
            {
                var assembliesPath = ParseSemicolonDelimitedNames(lobSystemProperties[lobSystemName]);

                var assemblies = new LobSystemAssemblies();

                //Add full path to assembly names and load them.
                for (var i = 0; i < assembliesPath.Length; i++)
                {
                    try
                    {
        ...
        private static string[] GetResourceModelFilesPaths(SPFeatureProperty featureProperty, string featureFolder)
        {
            ...
            for (var i = 0; i < resourceFilesPaths.Length; i++)
            {
                try
                {
            ...
            Dictionary<string, LobSystemAssemblies> lobSystemAssemblies, bool incrementalUpdate)
        {
            ...
            try
            {
                ...
                else if (GetModel(modelName) == null)
                {
                    upgrade = false;
                }
                ...
                foreach (var lobSystem in collection)
                {
                    if (!lobSystemAssemblies.TryGetValue(lobSystem.Name, out lobassemblies)) continue;
        ...
        private Model ImportModel(string fileContents, PackageContents packageContents, bool incrementalUpdate)
        {
            ...
            if (errors != null && errors.Length > 0)
            {
                ...
                var allEntities = new List<Entity>(model.AllEntities);

                if (allEntities.Count > 0)
                {
                    var activationErrors = new StringBuilder();
                    activationErrors.AppendLine();

                    var entityErrors = Entity.Validate(allEntities, this.amc);
                    ...
                    if (entityErrors.Length > 0)
                    {
                        throw new ArgumentException(String.Format(CultureInfo.CurrentCulture,
                                                                  ...
                                                                  this.modelFileName, activationErrors), "ModelFileName");
                    }
                }
            }
        ...
        private string GetDefaultWebAppUrl()
        {
            ...
            foreach (var uri in webServices.SelectMany(webService => (from webApp in webService.WebApplications let uri = webApp.GetResponseUri(SPUrlZone.Default) where !webApp.IsAdministrationWebApplication && ((uri.Port == 80 && uri.Scheme.ToUpperInvariant().Equals("HTTP")) || (uri.Port == 443 && uri.Scheme.ToUpperInvariant().Equals("HTTPS"))) select uri)))
            {
                if (result == null)
                {
                    conflict = false;
                ...
                else
                {
                    ...
                    {
                        var uriIsWww = uri.Segments[0].ToUpperInvariant().Equals("WWW");
                        var resultIsWww = result.Segments[0].ToUpperInvariant().Equals("WWW");
        ...
        private static byte[] GetAssemblyBytes(string assemblyPath)
        {
            ...
            using (var fs = new FileStream(assemblyPath, FileMode.Open, FileAccess.Read))
            {
                ...
                {
                    throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
                                                              "Assembly file '{0}' exceeds the maximum length of {1}.",
                                                              assemblyPath, int.MaxValue));
                }
            }
        ...
        private static string[] ParseSemicolonDelimitedNames(string value)
        {
            ...
            var fileNames = value.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries);
